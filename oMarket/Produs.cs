...
namespace oMarket
{
    public partial class Produs : Form
    {
        ...
        {
            ModulProdus productModule = new ModulProdus(this);
        ...
        private void dgvProduct_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Edit")
            {
                ModulProdus product = new ModulProdus(this);
            ...
            {
                if (MessageBox.Show("Vreti sa stergeti inregistrarea?", "Stergere inregistrare", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbProduct WHERE pcode LIKE '" + dgvProduct[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Produsul a fost sters.", "oMarket", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            LoadProduct();
        }
