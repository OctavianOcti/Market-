...
namespace oMarket
{
    public partial class GenereazaPlata : Form
    {
        SqlConnection cn = new SqlConnection();
        SqlCommand cm = new SqlCommand();
        DBConnect dbcon = new DBConnect();        
        MainformCasier cashier;
        public GenereazaPlata(MainformCasier cash)
        {
            InitializeComponent();
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Cantitate insuficienta, introduceti o cantitate corecta!", "Atentionare", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
                else
                {
                    ...
                    Factura recept = new Factura(cashier);
                    recept.LoadRecept(txtCash.Text, txtChange.Text);
                    recept.ShowDialog();

                    MessageBox.Show("Plata a fost salvata!", "Plata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    cashier.GetTranNo();
                    cashier.LoadCart();
                    this.Dispose();
                }
