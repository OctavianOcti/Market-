...
namespace oMarket
{
    public partial class Furnizor : Form
    {
        ...
        {
            ModulFurnizor supplierModule = new ModulFurnizor(this);
        ...
        private void dgvSupplier_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Stergeti inregistrarea?", "Confirmare", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("Delete from tbSupplier where id like '" + dgvSupplier.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Inregistrarea a fost stearsa", "Stergere inregistrare", MessageBoxButtons.OK, MessageBoxIcon.Information);
