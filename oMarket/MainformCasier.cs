...
namespace oMarket
{
    public partial class MainformCasier : Form
    {
        ...
        string stitle = "oMarket";
        public MainformCasier()
        {
            InitializeComponent();
        ...
        {
            if (MessageBox.Show("Parasire aplicatie?", "Confirmare", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                Application.Exit();
            }
        }
        ...
        {
            slide(btnSearch);
            CautaProdus lookUp = new CautaProdus(this);
            lookUp.LoadProduct();
            lookUp.ShowDialog();
        }
        ...
        {
            slide(btnSettle);
            GenereazaPlata settle = new GenereazaPlata(this);
        ...
        {
            slide(btnDSales);
            RaportVanzari dailySale = new RaportVanzari(new MainFormAdmin());
        ...
        {
            slide(btnPass);
            SchimbaParola change = new SchimbaParola(this);
            change.ShowDialog();
        }
        ...
        public void AddToCart(string _pcode, double _price,int _qty)
        {
            try
            {
                ...
                if (found)
                {
                    ...
                    {
                        MessageBox.Show("Eroare. Cantitatea ramasa este " + qty, "Eroare", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    cn.Open();
                ...
                else
                {
                    if (qty < (int.Parse(txtQty.Text) + cart_qty))
                    {
                        MessageBox.Show("Eroare. Cantitatea ramasa este " + qty, "Atentionare", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                    cn.Open();
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Sterge acest element", "Sterge element", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.ExecuteQuery("Delete from tbCart where id like'" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Elementele au fost sterse", "Stergere element", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadCart();
                }
            }
            else if (colName == "colAdd")
            {
                ...
                {
                    MessageBox.Show("Cantitate ramasa " + i + "!", "Stoc epuizat", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
            else if (colName == "colReduce")
            {
                ...
                {
                    MessageBox.Show("Cantitate ramasa " + i + "!", "Atentionare", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }
            }
        }
        ...
        public void Noti()
        {
            ...
            {
                i++;
                Alert alert = new Alert(new MainFormAdmin());
