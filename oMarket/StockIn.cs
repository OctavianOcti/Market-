...
namespace oMarket
{
    public partial class StockIn : Form
    {
        ...
        MainFormAdmin main;
        public StockIn(MainFormAdmin mn)
        {
            InitializeComponent();
        ...
        private void btnEntry_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (MessageBox.Show("Sunteti sigur ca vreti sa salvati inregistrarea?", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
        ...
        private void dgvStockIn_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Stergeti elementul?", stitle, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbStockIn WHERE id='" + dgvStockIn.Rows[e.RowIndex].Cells[1].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Elementul a fost sters", stitle, MessageBoxButtons.OK, MessageBoxIcon.Information);
                    LoadStockIn();
                }
            }
        }
