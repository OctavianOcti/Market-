...
namespace oMarket
{
    public partial class Categorie : Form
    {
        ...
        {
            ModulCategorie module = new ModulCategorie(this);
            module.ShowDialog();
        }
        ...
        private void dgvCategory_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (colName == "Delete")
            {
                if (MessageBox.Show("Sunteti sigur ca vreti sa stergeti inregistarea?", "Stergere inregistrare", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    cn.Open();
                    cm = new SqlCommand("DELETE FROM tbCategory WHERE id LIKE '" + dgvCategory[1, e.RowIndex].Value.ToString() + "'", cn);
                    cm.ExecuteNonQuery();
                    cn.Close();
                    MessageBox.Show("Categoria a fost stearsa", "Point Of Sales", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
